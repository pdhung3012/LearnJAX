...
y_np = np.array([[8.1], [10.9], [14.2], [17.0], [19.8],
                 [23.1], [26.0], [28.9], [32.2], [34.8]], dtype=np.float32)
...
def test_convergence():
    ...
    # PyTorch — pass writer=None equivalent by creating a dummy
    # b7's pt_train_model requires a writer argument
    ...
    pt_train_model(torch.tensor(X_np), torch.tensor(y_np),
                   model, optimizer, criterion, num_epochs, dummy_writer)
    pt_w = model.linear.weight.data.numpy()
    pt_b = model.linear.bias.data.numpy()

    # JAX — pass writer=None to skip TensorBoard logging
    jax_params = make_jax_params(W_np, B_np)
    jax_final = jax_train_model(jnp.array(X_np), jnp.array(y_np),
                                jax_params, num_epochs, lr, writer=None)
