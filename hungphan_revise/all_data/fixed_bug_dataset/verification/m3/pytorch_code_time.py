...
def train_test_loop(model, train_loader, test_loader, epochs=10):
    ...
    with torch.no_grad():
        for image_test, label_test in test_loader:
            image_test = image_test.to(
                device, non_blocking=(device.type == "cuda"))
            label_test = label_test.to(
                device, non_blocking=(device.type == "cuda"))
...
def config_init(init_type="kaiming"):
    ...
    initializer_dict = {"kaiming": kaiming_init, "xavier": xavier_init,
                        "zeros": zeros_init, "random": random_init}
...
for name, model in zip(
    ...
    [
        VanillaCNNModel().to(device),
        VanillaCNNModel().apply(config_init("kaiming")).to(device),
        VanillaCNNModel().apply(config_init("xavier")).to(device),
        VanillaCNNModel().apply(config_init("zeros")).to(device),
        VanillaCNNModel().apply(config_init("random")).to(device),
    ],
):
