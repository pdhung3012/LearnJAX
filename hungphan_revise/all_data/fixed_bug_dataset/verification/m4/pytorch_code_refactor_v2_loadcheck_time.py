...
ct_images = torch.from_numpy(np.load("ct_images.npy")).float().to(device)
segmentation_masks = torch.from_numpy(
    np.load("segmentation_masks.npy")).float().to(device)
...
model = MedCNN(backbone=resnet_model).to(device)
