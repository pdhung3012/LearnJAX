...
segmentation_masks = (torch.randn(
    size=(batch, num_slices, 1, width, height)) > 0).float()

np.save('ct_images', ct_images.detach().cpu().numpy().astype(np.float32))
np.save('segmentation_masks',
        segmentation_masks.detach().cpu().numpy().astype(np.float32))
