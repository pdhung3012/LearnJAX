...
def generate_data(num_samples=100):
    key = random.PRNGKey(42)  # ← Change from 0 to 42
    key, subkey = random.split(key)
    X = random.uniform(subkey, shape=(num_samples, 1)) * 10  # ← Change from linspace
    key, subkey = random.split(key)
...
def train_model(X, y, num_epochs=1000):
    key = random.PRNGKey(42)  # ← Change from 0 to 42
    k = 1.0  # 1/in_features
    bound = jnp.sqrt(k)  # ← Match PyTorch's default init
    key, subkey = random.split(key)
