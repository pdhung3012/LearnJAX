...
import numpy as np
np.save('src_data', src_data.detach().cpu().numpy().astype(np.int32))
np.save('tgt_data', tgt_data.detach().cpu().numpy().astype(np.int32))
...
np.save('test_input', test_input.detach().cpu().numpy().astype(np.int32))
