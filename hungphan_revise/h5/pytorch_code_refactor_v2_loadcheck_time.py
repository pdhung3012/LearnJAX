...
import numpy as np
src_data=torch.from_numpy(np.load("src_data.npy")).long()
tgt_data=torch.from_numpy(np.load("tgt_data.npy")).long()
...
test_input=torch.from_numpy(np.load("test_input.npy")).long()
