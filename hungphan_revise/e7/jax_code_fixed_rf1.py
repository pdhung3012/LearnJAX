...
def main():
    ...
    import numpy as np
    X_train = jnp.asarray(np.load("X.npy"))
    y_train = jnp.asarray(np.load("y.npy"))
    ...
    X_test = jnp.asarray(np.load("X_test.npy"))

    model = SimpleModel()  # Initialize model
    predictions = model.apply(loaded_params, X_test)  # Get predictions
    print(f"Predictions after loading: {predictions}")  # FIXED: Changed message to match PyTorch
