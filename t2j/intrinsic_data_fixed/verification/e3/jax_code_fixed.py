...
def main():
    ...
    # FIXED: Plot actual model output with custom activation, not just linear component
    X_sorted = jnp.sort(X, axis=0)
    predictions_sorted = model(params, X_sorted)
    plt.plot(X_sorted, predictions_sorted, 'r', label='Model Fit')
    plt.legend()
    plt.show()
